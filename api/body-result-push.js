// ============================================================
// 抗老研究室｜體態問卷正式 LINE 串接
// Vercel Serverless Function: /api/body-result-push.js
//
// 只接受 { idToken, answers }。
// 後端重新向 LINE 驗證身分、重新用 lib/surveys/body.js 計算結果，
// 絕不信任前端提供的 userId / resultKey / headline / copyText / message。
// ============================================================
...
module.exports = async function handler(req, res) {
  ...
  // 2) 後端用同一套共用計分邏輯重新計算正式體態結果（不信任前端傳來的任何結果欄位）
  let result;
  try {
    result = calculateBodyResult(answers);
  } catch (e) {
  ...
  try {
    ...
    if (!pushRes.ok) {
      console.error('[body-result-push] push failed', {
    ...
    console.error('[body-result-push] push exception', { endpoint: 'v2/bot/message/push' });
    return res.status(502).json({ ok: false, error: 'LINE_PUSH_FAILED' });
  }
};

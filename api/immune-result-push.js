// ============================================================
// 抗老研究室｜免疫問卷正式 LINE 串接
// Vercel Serverless Function: /api/immune-result-push.js
//
// 只接受 { idToken, answers }。
// 後端重新向 LINE 驗證身分、重新用 lib/surveys/immune.js 計算結果，
...
module.exports = async function handler(req, res) {
  ...
  // 2) 後端用同一套共用計分邏輯重新計算正式免疫結果（不信任前端傳來的任何結果欄位）
  let result;
  try {
    result = calculateImmuneResult(answers);
  } catch (e) {
  ...
  try {
    ...
    if (!pushRes.ok) {
      console.error('[immune-result-push] push failed', {
    ...
    console.error('[immune-result-push] push exception', { endpoint: 'v2/bot/message/push' });
    return res.status(502).json({ ok: false, error: 'LINE_PUSH_FAILED' });
  }
};

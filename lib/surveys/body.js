// ============================================================
// 抗老研究室｜體態管理檢測 - 共用計分邏輯
// 從 api/survey/body.js 抽出，供 API handler 與之後的 LINE Push
// 共用同一套計分來源，計分規則與文案內容均未變動。
// ============================================================
...
// 注意：這只給 copyText 這組「複製用固定文字」的顯示排列使用，
// 不影響、也不能拿來決定真正的 primaryKey / secondaryKey（見 rankBodyTypes()）。
...
// 計算某個 type 在這次作答裡的原始分數，以及 3 分 / 2 分答案各有幾題。
// 只是把已經存在的 raw score 加總邏輯，順便算出兩個 tie-break 用的計數，
// 不會改變 raw score 本身的算法。
function getAnswerDistributionForType(questions, answers) {
  let score = 0, count3 = 0, count2 = 0;
  ...
  return { score, count3, count2 };
}

// 六型「完全平等競爭」排名：
// 1) 總分高者優先
// 2) 總分同分 → 3 分答案數量多者優先
// 3) 3 分答案數量也同分 → 2 分答案數量多者優先
// 4) 以上全部相同 → 才用 TYPE_ORDER 目前既有的順序當 deterministic fallback，
//    確保完全同分時每次重算結果一致；這一層 fallback 不代表任何類型比較「優先」，
//    純粹是為了讓輸出穩定。
function rankBodyTypes(answers) {
  const fallbackOrder = Object.fromEntries(TYPE_ORDER.map((key, i) => [key, i]));
  const stats = Object.fromEntries(
    Object.entries(GROUPS).map(([key, { questions }]) => [key, getAnswerDistributionForType(questions, answers)])
  );

  return Object.keys(GROUPS).sort((a, b) => {
    if (stats[b].score !== stats[a].score) return stats[b].score - stats[a].score;
    if (stats[b].count3 !== stats[a].count3) return stats[b].count3 - stats[a].count3;
    if (stats[b].count2 !== stats[a].count2) return stats[b].count2 - stats[a].count2;
    return fallbackOrder[a] - fallbackOrder[b];
  });
}
...
function calculateBodyResult(answers) {
  const scores = {};
  for (const [group, { questions }] of Object.entries(GROUPS)) {
    scores[group] = questions.reduce((sum, q) => sum + (answers[q] ?? 0), 0);
  }

  // 實際主次：六型完全平等競爭，依 rankBodyTypes() 的排名結果決定，
  // 不再讓 stable sort 在同分時預設用 GROUPS 宣告順序（那會讓宣告在最後的 gene 每次同分必輸）。
  const ranked = rankBodyTypes(answers);
  ...
  // isDualPrimary 只看「總分差」，不管 tie-break 是靠 count3/count2/fallback 決定出來的名次。
...
module.exports = { calculateBodyResult };

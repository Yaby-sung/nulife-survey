// ============================================================
// 抗老研究室｜肌膚問卷正式 LINE 串接
// Vercel Serverless Function: /api/skin-result-push.js
//
// 只接受 { idToken, answers }。
// 後端重新向 LINE 驗證身分、重新用 lib/surveys/skin.js 計算結果，
// 絕不信任前端提供的 userId / resultKey / headline / message / shortName。
// ============================================================
...
module.exports = async function handler(req, res) {
  ...
  // 2) 後端用同一套共用計分邏輯重新計算正式肌膚結果（不信任前端傳來的任何結果欄位）
  // 即使 answers 缺少某些題號（例如 Q2 選填未答），也直接交給 calculateSkinResult 依既有規則處理，不預先補值。
  let result;
  try {
    result = calculateSkinResult(answers);
  } catch (e) {
  ...
  try {
    ...
    if (!pushRes.ok) {
      console.error('[skin-result-push] push failed', {
    ...
    console.error('[skin-result-push] push exception', { endpoint: 'v2/bot/message/push' });
    return res.status(502).json({ ok: false, error: 'LINE_PUSH_FAILED' });
  }
};
